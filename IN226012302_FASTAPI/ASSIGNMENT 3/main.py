...
def add_product(new_p: NewProduct, response: Response):
    # Check for duplicate name (Q1)
    if any(p["name"].lower() == new_p.name.lower() for p in products):
        response.status_code = status.HTTP_400_BAD_REQUEST
        return {"error": f"Product '{new_p.name}' already exists"}
    
    # Auto-generate ID
    new_id = max(p["id"] for p in products) + 1 if products else 1
    product_dict = new_p.dict()
...
def product_audit():
    ...
    # Q5 Logic: price * 10 units for in-stock
...
def update_product(
    ...
    # Q2 Logic: Only update if value is sent
